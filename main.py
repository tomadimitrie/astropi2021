...
if __name__ == "__main__":
    try:
        ...
        logfile(dir_path / "program.log")
        with open(dir_path / "data01.csv", "w") as file:
        ...
        with open(dir_path / "data02.csv", "w") as file:
        ...
        while now_time < start_time + datetime.timedelta(minutes=170):
            ...
            if time_period == TimePeriod.DAY:
                ...
                cam.capture(dir_path / "images" / f"{photo_count}.png", format="png")
                with open(dir_path / "data02.csv", "a") as file:
                    csv.writer(file).writerow((photo_count, iss.sublat, iss.sublong, location, now_time))
                time.sleep(2 * 60)
            else:
                if computed_photo_count <= photo_count:
                    # compute the data
                    light_directions = compute_light_directions.compute(
                        dir_path / "images" / f"image{computed_photo_count}.png"
                    )
                    compute_albedo_map.compute(
                        dir_path / "images" / f"image{computed_photo_count}.png",
                        light_directions
                    )
                    albedo = compute_albedo.compute(dir_path / "images" / f"image{computed_photo_count}.png")
                    energy, temperature = compute_temperature.compute_temperature_no_greenhouse(albedo)
                    optimized_albedo = compute_albedo.compute(
                        dir_path / "output_albedos" / f"image{computed_photo_count}.png"
                    )
                    ...
                    with open(dir_path / "data01.csv", 'a') as file:
                        writer = csv.writer(file)
                        writer.writerow((
                    ...
                    logger.info(
                        f"computed image with index {computed_photo_count} "
                        f"started at {now_time} and ended at {datetime.datetime.now()}"
                    )
