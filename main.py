    ...
    try:
        ...
        while now_time < start_time + datetime.timedelta(minutes=170):
            ...
            if time_period == TimePeriod.DAY:
                ...
                logger.info(
                        "took photo with index " + str(photo_count) + " started at " + str(now_time) + " and ended at " + str(datetime.datetime.now())
                    )
                time.sleep(2 * 60)
                photo_count += 1
            else:
                if computed_photo_count <= photo_count and photo_count != 0:
                    ...
                    logger.info(
                        "computed light directions with index " + str(computed_photo_count) + " started at " + str(now_time) + " and ended at " + str(datetime.datetime.now())
                    )
                    if light_directions is not None:
                        ...
                        logger.info(
                            "computed albedo map with index " + str(computed_photo_count) + " started at " + str(now_time) + " and ended at " + str(datetime.datetime.now())
                        )
                    ...
                    logger.info(
                        "computed image with index " + str(computed_photo_count) + " started at " + str(now_time) + " and ended at " + str(datetime.datetime.now())
                    )
